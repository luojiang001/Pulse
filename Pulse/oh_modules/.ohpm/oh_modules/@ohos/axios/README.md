  ...
  ```javascript
{
    ...
    // Use AbortController to cancel the axios request
...
```javascript
{
  ...
  // headers indicates an array of HTTP headers with which the server responds.
...
```
Default request header setting authorization
```javascript
// Set the default request header authorization field
axios.setHeaderKey('authorization', value);
    
// Delete the authorization field in the default request header according to the key
...
Default value of custom instance request header
```javascript
// Modify default values after instance creation
axios.setHeaderKey('authorization', value, axios);
    
// After creating an instance, delete the authorization
...
### Set up the use of national security SSL, and when sslType: 'TLCP', the fields for client EncCert and client Cert must be valid (API20 or above)
...
axios<InfoModel, AxiosResponse<InfoModel>, null>({
	...
	// the logic for successfully processing requests
}).catch((err: AxiosError) => {
	// logic of failed request processing
})
```
...
```javascript
  axios<infoModel, AxiosResponse<infoModel>, null>({
    ...
    usingProtocol: http.HttpProtocol.HTTP2, // Set usage protocol
  }).then((res: AxiosResponse) => {
      // 
  }).catch((err: AxiosError) => {
      //
...
// Download. If the file already exists, delete it first.
try {
...
### Set the server using https protocol for DNS resolution
```javascript
  axios<infoModel, AxiosResponse<infoModel>, null>({
...
### Set the specified DNS server for DNS resolution
```javascript
  axios<infoModel, AxiosResponse<infoModel>, null>({
    url: 'https://www.xxx.com/xx',
    method: 'get',
...
#### Obtaining the Message Digest Through the digest Field（PIN Code）
...
#### Scheme I：Add a new configuration file, and configure the relevant information of the certificate in the file: the path of the configuration file is: **entry/src/main/resources/base/profile/network_config.json**.

Configuration file name: network_config  
Note: This configuration file is a global configuration and will lock all certificates
...
#### Scheme II: Set certificatePinning locking configuration dynamically
```javascript
  axios<infoModel, AxiosResponse<infoModel>, null>({
    url: 'https://www.xxx.com/xx',
    method: 'get',
    certificatePinning: {
        publicKeyHash: 'PIN Code', // The certificate PIN code passed in by the application has supported this attribute since API 12.
        hashAlgorithm: 'SHA-256' // The encryption algorithm currently only supports SHA-256, which has been supported since API 12.
    },
    caPath: '',
    clientCert: {
        certPath: '',
...
### Set server authentication configuration during secure connection
```javascript
  axios<infoModel, AxiosResponse<infoModel>, null>({
    url: 'http://www.xxx.com/xx',
    method: 'get',
...
// Define the onabort event
abortController.signal.onabort = (rea: ESObject) => {
    console.info('abort', JSON.stringify(rea))
}
// Use the AbortController.abort() method to cancel the request
// The abort() method can pass in a custom reason: object; It will be passed to the onabort event
...
| ERR_CANCELED | string | Yes| No| Feature or method is canceled explicitly by the user using an AbortSignal.|
...
> **NOTE**:<br>
> 1.The certificate-based bidirectional authentication(clientCert) and certificate pinning features are available only in API version 11+, configure RemoteValidation(RemoteValidation) are available only in API version 18+, Other features are available since API version 9.<br>
> 2.The signal attribute can only cancel requests that have not yet been actually sent. For a request that has already been sent, calling abort() will immediately trigger an error and enter.catch(), and attempt to terminate the request through the underlying connection interrupt (destroy()). However, due to the Network kit processing mechanism, it cannot be guaranteed that the request will be stopped immediately.
