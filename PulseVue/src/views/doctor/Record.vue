...
const doctors=()=>{
  axios.get("doctor/list").then(res => {
    doctorList.value = res.data.filter((item: any) => {
      const doctorId = Number(item.doctor?.id);
      const currentDocId = Number(doctor.value.data.id);
      return (doctorId === currentDocId) && (item.visitStatus === 0);
    })
  }).catch(() => {
...
const showDetail = (row:any) => {
  console.log('正在请求详情:', row.regNo);
  axios.get(`registrationRecord/detail/${row.regNo}`).then(res => {
    console.log('详情请求返回:', res.data);
    if (res.data.code === 200) {
      detailForm.value = res.data.data;
      console.log('detailForm赋值后:', detailForm.value);
